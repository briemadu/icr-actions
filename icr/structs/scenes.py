...
class Scene:
    ...
    def __init__(self, scene: str):
        ...
        self.cliparts = self._get_cliparts()
        self.is_empty = self._is_empty()

    def _get_cliparts(self) -> List[ClipArt]:
    ...
    def clipart_positions(self) -> List[Tuple[str, float, float]]:
        ...
        return [(obj.png, obj.x, obj.y) for obj in self.cliparts]

    @property
    def n_present_cliparts(self) -> int:
        """Return the number of objects present in a scene."""
        return len([x for x in self.cliparts if x.exists])
...
class SceneSeq:
    ...
    def gallery(self) -> List[str]:
        """List the cliparts in the gallery."""
        turn = 0
        # sometimes, scene strings are empty
        # but the gallery shouldn't change, so we retrieve it
        # from the first turn that contains a filled scene string
        while True:
            scene = self.seq[turn]
            if not scene.is_empty:
                break
            turn += 1
        return [clip.name for clip in scene.cliparts]
